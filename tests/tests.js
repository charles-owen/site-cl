...
var Tests = function (id, resultsid) {
	 ...
	 * @param test The name of the class to test */
	...
	this.add = function(test, name) {
		this.tests.push({test: test, name: name});
	}
	...
	this.present = function() {
		...
		html = '';
		for(i in this.tests) {
			var name = this.tests[i]['name'];
            var test = this.tests[i]['test'];
			var t = test;
			var tt = t + "Test";

			let regex = new RegExp('[^/]*$');
			t = name + '/' + t.match(regex)[0];
	...
	this.do_all = function() {
		...
		for(test in this.tests) {
			this.to_run.push(this.tests[test].test + 'Test');
		}
		this.start();
	}
